...
def _home_ratio(state: list[float], home_pose: list[float], threshold: float) -> float:
    width = min(len(state), len(home_pose))
    if width <= 0:
        return 0.0
    close = sum(1 for dim in range(width) if abs(state[dim] - home_pose[dim]) <= threshold)
    return close / width
...
def _segment_edge_mask(
    ...
    *,
    ...
    for index, state in enumerate(states):
        near_home = _home_ratio(state, home_pose, joint_threshold) >= min_home_ratio
...
def _score_boundaries(
    ...
    for index, state in enumerate(states):
        ...
        ratio = _home_ratio(state, home_pose, args.joint_threshold)
...
def split_batch(args: argparse.Namespace) -> dict[str, Any]:
    specs = _parse_specs(args.subtasks, args.task_preset)
    if not specs:
        raise ValueError("At least one subtask is required")

    source_roots = [Path(item) for item in args.source_root]
    default_output_root = source_roots[0].expanduser().resolve()
    ...
    if args.split_videos and shutil.which(args.ffmpeg) is None and not Path(args.ffmpeg).exists():
